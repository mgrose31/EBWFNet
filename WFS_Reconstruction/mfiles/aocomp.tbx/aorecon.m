function [recon, rsv, ru, rs, rv, rb] = aorecon(mvtos, nsingin, remtiltin, nsubin)
% SYNTAX:
% [recon, rsv] = aorecon(slpif, nsing, {remtilt})
% rsv = aorecon(slpif)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% DESCRIPTION: 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% INPUTS:
% slpif [ ] = The 2*nsub x nact slope influence function matrix. nsub is 
%             the  number of subapertures and nact is the number of 
%             (master) actuators. The rows must be arranged such that nsub 
%             x-slopes are followed by nsub y-slopes.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% $Id: aorecon.m 3027 2010-09-21 21:04:10Z amoran $
...
if (2*floor(nsub2/2) ~= nsub2)
   warning(['aorecon: The inner dimension of slpif must be even.']);
   help aorecon;
   return;
else
   nsub = nsub2/2;
